...
if ($totalpages == 1) {
    // If there's only one page, show it as active
    ...
    // Show ellipsis if the current page is not close to the start
    if ($pvalue > 4) {
        $output .= '<li>...</li>';
    }

    // Show 3 pages before and after the current page
    $start = max(1, $pvalue - 3);
    $end = min($totalpages, $pvalue + 3);
    for ($i = $start; $i <= $end; $i++) {
